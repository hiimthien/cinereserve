    ...
    <div>
      ...
      <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-cinema-border pb-6">
          <div class="space-y-2">
            <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-emerald-400">
              <Flame class="w-4 h-4 text-emerald-400 animate-pulse" />
              <span>Đang Chiếu Tại Các Rạp Toàn Quốc</span>
            </div>
            <h1 class="text-3xl md:text-4xl font-black text-white tracking-tight">
              Phim Đang Chiếu
            </h1>
            <p class="text-sm text-cinema-muted">
              Khám phá các bộ phim bom tấn đang làm mưa làm gió tại phòng vé Việt Nam hôm nay.
            </p>
          </div>
        ...
        <div class="flex items-center gap-1.5 overflow-x-auto pb-2 pt-1 scrollbar-none select-none -mt-4">
          <button
            ...
            :class="[
              selectedGenre === g.id
                ? 'bg-emerald-500/25 text-emerald-300 border-emerald-400/60 font-bold shadow-sm backdrop-blur-md scale-105'
...
const filteredMovies = computed(() => {
  ...
  return store.nowShowingMovies.filter(m => {
